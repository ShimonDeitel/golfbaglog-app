...
struct ItemRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text("Loft: \(item.primaryField)")
                .font(Theme.captionFont)
                .foregroundColor(Theme.textMuted)
            if !item.secondaryField.isEmpty {
                Text("Shaft Flex: \(item.secondaryField)")
                    .font(Theme.captionFont)
                    .foregroundColor(Theme.textMuted)
            }
            if !item.subLogs.isEmpty {
                Text("\(item.subLogs.count) Round Notes entries")
                    .font(Theme.captionFont)
        ...
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    TextField("Loft", text: $primaryField)
                        .accessibilityIdentifier("primaryFieldInput")
                    TextField("Shaft Flex", text: $secondaryField)
                ...
                Section("Round Notes") {
                    if purchases.isPurchased {
                        ...
                        TextField("New round note", text: $newSubLogNote)
                        ...
                        Button("Unlock Round Notes with Pro") {
                            showingPaywall = true
                        }
